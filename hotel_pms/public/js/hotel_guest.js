(()=>{
  ...
  async function laundryRequest(){
    const notes=prompt('Pickup notes, item estimate, and preferred pickup time');if(!notes)return;
    try{const data=await call('hotel_pms.guest_portal.guest_request_laundry',{raw_token:token,notes,request_key:crypto.randomUUID()},true);msg(`Laundry pickup request ${data.laundry_order} submitted.`)}catch(error){msg(error.message)}
  }
  async function loadExperiences(){
    try{const data=await call('hotel_pms.guest_portal.guest_list_experiences',{raw_token:token},true),target=document.getElementById('experience-list');target.innerHTML=data.experiences.length?data.experiences.map(e=>`<div class="border-top pt-2 mt-2"><b>${esc(e.experience_name)}</b> · ${Number(e.price).toLocaleString()}<br>${esc(e.description||'')}<br><button class="hotel-btn secondary exp-book" data-name="${esc(e.name)}">Request booking</button></div>`).join(''):'No public experiences are available.';target.querySelectorAll('.exp-book').forEach(b=>b.onclick=()=>bookExperience(b.dataset.name))}catch(error){msg(error.message)}
  }
  async function bookExperience(name){const when=prompt('Schedule (YYYY-MM-DD HH:MM:SS)');if(!when)return;const participants=Number(prompt('Participants','1')||1);try{const data=await call('hotel_pms.guest_portal.guest_book_experience',{raw_token:token,experience:name,scheduled_at:when,participants,notes:'Requested through guest portal',request_key:crypto.randomUUID()},true);msg(`Experience request ${data.experience_booking} submitted.`)}catch(error){msg(error.message)}}
