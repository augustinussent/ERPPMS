...
def main():
    p=argparse.ArgumentParser();p.add_argument('--url',required=True);p.add_argument('--payload',required=True);p.add_argument('--requests',type=int,default=10);p.add_argument('--expected-successes',type=int,required=True);p.add_argument('--token',default='');a=p.parse_args();payload=json.load(open(a.payload));results=[]
    ...
    report={"requests":a.requests,"successes":len(successes),"expected_successes":a.expected_successes,"unique_refs":len(set(refs)),"statuses":[x[0] for x in results]};print(json.dumps(report,indent=2))
