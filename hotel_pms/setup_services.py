...
KOT_HTML = r"""<style>@page{size:80mm auto;margin:3mm}body{font-family:monospace;font-size:11px}.c{text-align:center}.line{border-top:1px dashed #000;margin:5px 0}.item{font-size:14px;font-weight:bold}.alert{font-size:13px;font-weight:bold;border:2px solid #000;padding:3px;margin:3px 0}</style><div class="c"><b>{{ doc.outlet }}</b><br>KOT #{{ doc.daily_kot_number }} · {{ doc.kitchen_station }}<br>{{ doc.restaurant_order }} · {{ doc.sent_at }}</div><div class="line"></div><div>{{ doc.table or doc.room or '-' }} · {{ doc.guest_name or '-' }}<br>Captain: {{ doc.captain or '-' }} · {{ doc.priority or 'Normal' }} · {{ doc.course or '-' }}</div><div class="line"></div>{% for row in doc.items %}<div class="item">{{ row.qty }} × {{ row.item_name }}</div>{% if row.course %}<div>Course: {{ row.course }}</div>{% endif %}{% if row.allergy_alert %}<div class="alert">ALLERGY: {{ row.allergy_alert }}</div>{% endif %}{% if row.notes %}<div>{{ row.notes }}</div>{% endif %}{% endfor %}<div class="line"></div>"""
