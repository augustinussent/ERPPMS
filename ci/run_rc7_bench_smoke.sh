...
: "${RC7_TEST_PROPERTY:?Set RC7_TEST_PROPERTY to an existing Hotel Property on the disposable site}"
APP_SOURCE=${GITHUB_WORKSPACE:-$(cd "$(dirname "$0")/.." && pwd)}
EVIDENCE_DIR=${VALIDATION_EVIDENCE_DIR:-$APP_SOURCE/validation-evidence}
mkdir -p "$EVIDENCE_DIR"
EVIDENCE_FILE="$EVIDENCE_DIR/rc7-control-smoke-$(date -u +%Y%m%dT%H%M%SZ).log"
{
  cd "$BENCH_PATH"
  ...
  bench --site "$SITE" run-tests --module hotel_pms.tests.test_intelligence_rc7_integration
  bench --site "$SITE" execute hotel_pms.intelligence_ci.run_rc7_smoke --kwargs "{\"property_name\":\"$RC7_TEST_PROPERTY\"}"
  if [[ -n "${GATE_RUN:-}" ]]; then
    bench --site "$SITE" execute hotel_pms.staging_execution.run_smoke_suite --kwargs "{\"gate_run\":\"$GATE_RUN\"}"
  else
    echo "Gate run not supplied; RC7 smoke completed without Production Gate attachment."
  fi
