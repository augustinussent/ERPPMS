...
class HotelGroupFolioCharge(Document):
    def validate(self):
        self.amount = (self.qty or 0) * (self.rate or 0)
        profile_name = self.tax_profile
        if not profile_name and self.parent:
            property_name = frappe.db.get_value("Hotel Group Folio", self.parent, "property")
            profile_name = frappe.db.get_value("Hotel Property", property_name, "default_hotel_tax_profile") if property_name else None
        if not profile_name:
        ...
        row = tax_breakdown(self.amount, profile.service_charge_rate, profile.tax_rate, profile.tax_basis,
